...
export function clearAt(p,r=.34){return Math.abs(p.x)<LIMITS.x-r&&Math.abs(p.z)<LIMITS.z-r&&OBSTACLES.every(o=>Math.hypot(p.x-o.x,p.z-o.z)>o.r+r);}
...
export function canMount(s){
  ...
  return s.mode==='foot'&&Math.hypot(s.player.x-p.x,s.player.z-p.z)<1.15&&dx*Math.cos(s.bike.yaw)-dz*Math.sin(s.bike.yaw)<-.25&&clearAt(p,.4);
}
export function createMovement(){return{mode:'foot',player:{x:-1.5,z:.75,yaw:-1.05},bike:{x:0,z:0,yaw:0},speed:0,walkSpeed:0,walkDistance:0,phase:0,wheelAngle:0,steer:0,lean:0,clock:0,elapsed:0,paused:false,target:null,blocked:false,transition:null};}
export function interact(s){
  ...
  if(s.mode==='ride'&&Math.abs(s.speed)<.3&&clearAt(mountSpot(s),.42)){
    s.mode='settle';s.elapsed=0;s.speed=0;s.transition={phase:s.phase,target:Math.round(s.phase/(Math.PI*2))*Math.PI*2};return true;
  }
  return false;
}
export function setWalkTarget(s,p){if(s.mode==='foot'&&clearAt(p,.34)){s.target={x:p.x,z:p.z};return true;}return false;}
function walkClear(s,p){
  if(!clearAt(p))return false;
...
export function movementStatus(s){
  ...
  const near=canMount(s),canGetOff=s.mode==='ride'&&Math.abs(s.speed)<.3&&clearAt(mountSpot(s),.42);
