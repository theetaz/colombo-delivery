...
report={}
for name in ['Idle','Stand','Walk','Mount','Dismount','Pedal']:
    rig.animation_data.action=bpy.data.actions[name]
    low,high=map(int,rig.animation_data.action.frame_range)
    max_stretch=(0,0,[]);minimum=10
    frames=[f/2 for f in range(low*2,high*2+1)] if name in ('Walk','Stand') else range(low,high+1,3)
    for frame in frames:
        bpy.context.scene.frame_set(int(frame),subframe=frame%1);bpy.context.view_layer.update()
...
assert report['Walk']['lowestVertexMetres']>-.003,'Walking shoe penetrates the floor by more than 3 mm'

def knee(side):
    hip=rig.pose.bones['Thigh_'+side].head;ankle=rig.pose.bones['Foot_'+side].head;joint=rig.pose.bones['Shin_'+side].head
    return 180-math.degrees((hip-joint).angle(ankle-joint))
kinematics={}
for name in ['WalkBefore','Walk']:
    rig.animation_data.action=bpy.data.actions[name]
    elbows=[];swing=[];trunk=[]
    for i in range(101):
        frame=24*i/100;bpy.context.scene.frame_set(int(frame),subframe=frame%1);bpy.context.view_layer.update()
        for side in ['L','R']:
            upper=rig.pose.bones['UpperArm_'+side].head;elbow=rig.pose.bones['Forearm_'+side].head;hand=rig.pose.bones['Hand_'+side].head
            elbows.append(180-math.degrees((upper-elbow).angle(hand-elbow)))
            if .6<=(i/100+(0 if side=='L' else .5))%1:swing.append(knee(side))
        trunk.append(math.degrees((rig.pose.bones['Head'].head-rig.pose.bones['Pelvis'].head).angle(Vector((0,0,1)))))
    bpy.context.scene.frame_set(7,subframe=.2);bpy.context.view_layer.update()
    kinematics[name]={'supportKneeAt30PercentDegrees':knee('L'),'elbowFlexionDegrees':[min(elbows),max(elbows)],'peakSwingKneeDegrees':max(swing),'maxTrunkFromVerticalDegrees':max(trunk)}
report['walkingKinematics']=kinematics
report['assetSha256']=hashlib.sha256((ROOT.parent/'viewer/public/cyclist/courier-movement.glb').read_bytes()).hexdigest()
print(json.dumps(report,indent=2),flush=True)
assert all(v['lowestVertexMetres']>-.04 for v in report.values() if isinstance(v,dict) and 'lowestVertexMetres' in v),'A movement passes below the ground'
assert all(v['maxEdgeStretch'][0]<4 for v in report.values() if isinstance(v,dict) and 'lowestVertexMetres' in v),'Severe skin stretching in a new movement'
